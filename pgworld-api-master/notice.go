...
func NoticeGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + noticeTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + noticeTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
// NoticeAdd .
func NoticeAdd(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	fieldCheck := requiredFiledsCheck(body, noticeRequiredFields)
	if len(fieldCheck) > 0 {
	...
	// log
	logAction(body["admin_name"], "added notice "+body["name"], "3", body["hostel_id"])
	delete(body, "admin_name")

	body["status"] = "1"
	body["created_date_time"] = time.Now().UTC().String()

	status, ok := insertSQL(noticeTable, body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Notice added", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
...
func NoticeUpdate(w http.ResponseWriter, r *http.Request) {
	...
	for _, field := range noticeRequiredFields {
	...
	// log
	logAction(body["admin_name"], "updated notice "+body["name"], "3", body["hostel_id"])
	delete(body, "admin_name")

	body["modified_date_time"] = time.Now().UTC().String()

	status, ok := updateSQL(noticeTable, r.URL.Query(), body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Notice updated", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
