...
// FoodGet .
func FoodGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + foodTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + foodTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
// FoodAdd .
func FoodAdd(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	fieldCheck := requiredFiledsCheck(body, foodRequiredFields)
	if len(fieldCheck) > 0 {
	...
	// log
	// logAction(body["admin_name"], "added food", "4", body["food_id"])
	delete(body, "admin_name")

	body["status"] = "1"
	body["created_date_time"] = time.Now().UTC().String()

	status, ok := insertSQL(foodTable, body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Food added", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
...
func FoodUpdate(w http.ResponseWriter, r *http.Request) {
	...
	for _, field := range foodRequiredFields {
	...
	// log
	// logAction(body["admin_name"], "updated food", "4", body["food_id"])
	delete(body, "admin_name")

	body["modified_date_time"] = time.Now().UTC().String()

	status, ok := updateSQL(foodTable, r.URL.Query(), body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Food updated", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
