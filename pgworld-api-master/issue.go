...
func IssueGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + issueTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + issueTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
func IssueAdd(w http.ResponseWriter, r *http.Request) {
	...
	fieldCheck := requiredFiledsCheck(body, issueRequiredFields)
	if len(fieldCheck) > 0 {
	...
	// log
	logAction(body["admin_name"], "added issue "+body["name"], "3", body["hostel_id"])
	delete(body, "admin_name")

	body["status"] = "1"
	body["created_date_time"] = time.Now().UTC().String()

	status, ok := insertSQL(issueTable, body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Issue added", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
...
// IssueUpdate .
func IssueUpdate(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	for _, field := range issueRequiredFields {
	...
	// log
	logAction(body["admin_name"], "updated issue "+body["name"], "3", body["hostel_id"])
	delete(body, "admin_name")

	body["modified_date_time"] = time.Now().UTC().String()

	status, ok := updateSQL(issueTable, r.URL.Query(), body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Issue updated", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
